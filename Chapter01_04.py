...
DaemonThread(데몬스레드)
    (1). 백그라운드에서 실행
    (2). 메인스레드 종료시 즉시 강제 종료
    (3). 주로 백그라운드 무한 대기 이벤트 발생 실행하는 부분 담당 -> JVM(가비지 칼렉션), 자동 저장
    (4). 일반스레드는 작업 종료시 까지 실행
...
if __name__ == "__main__":
    ...
    y = threading.Thread(target = thread_func, args = ('Two', range(10)), daemon = True)
